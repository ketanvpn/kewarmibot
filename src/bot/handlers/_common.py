...
def is_owner(update: Update) -> bool:
    ...
    return owner_id(update) in {str(x) for x in settings.admin_ids}


async def cfg_dict() -> dict:
...
async def cookies_list():
    async with AsyncSessionLocal() as session:
        return await list_cookies(session)
...
def back_button(label: str = "« Menu Utama") -> InlineKeyboardButton:
